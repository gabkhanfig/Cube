...
#pragma region Append
		...
		constexpr TestString& append(const Str& str) {
			...
			if (isSso()) { // If it is an sso, it must reallocate.
				...
				copyChars(buffer + length, str.str, str.len);
		...
		constexpr TestString& append(const TestString& other) {
			...
			if (isSso()) { // If it is an sso, it must reallocate.
				...
				_rep._heap = HeapRep();
				uint64 mallocCapacity = minCapacity + otherLen; // overallocate
				char* buffer = mallocHeapBuffer(&mallocCapacity);

				copyChars(buffer, temp, length);
				copyChars(buffer + length, otherStr, otherLen);
			...
			copyChars(_rep._heap._buffer + length, otherStr, otherLen);
			setHeapLen(newLength);
			setHeapUsedBytes(capacityUsedNoTerminator + 1);
			return *this;
		}
...
#pragma region Concat
		...
		[[nodiscard]] constexpr friend TestString operator + (const TestString& lhs, const char rhs) {
			...
			uint64 mallocCapacity = minCapacity * 1.5; // overallocate
...
#pragma region Format
	...
	public:
		...
		constexpr static TestString format(const Types&... inputs) {
			static_assert(formatStrIsValid(formatStr), "String::format format str is not valid");
			...
			if (mallocCapacity <= MAX_SSO_UTF8_BYTES) {
				...
				output.setSsoLen(outputLength);
				output.setSsoUsedBytes(mallocCapacity - 1);
				return output;
			}
			...
			output.setHeapLen(outputLength);
			output.setHeapUsedBytes(mallocCapacity - 1);
			output.setHeapCapacity(currentStringIndex);
			return output;
		}
	...
	private:
		...
		[[nodiscard]] constexpr forceinline uint64 heapCapacity() const {			
			if (std::is_constant_evaluated()) {
				...
				heapCapacity |= (static_cast<uint64>(_rep._heap._totalByteCapacityBitmask[0]));
				heapCapacity |= (static_cast<uint64>(_rep._heap._totalByteCapacityBitmask[1]) << 8);
				heapCapacity |= (static_cast<uint64>(_rep._heap._totalByteCapacityBitmask[2]) << 16);
				heapCapacity |= (static_cast<uint64>(_rep._heap._totalByteCapacityBitmask[3]) << 24);
				heapCapacity |= (static_cast<uint64>(_rep._heap._totalByteCapacityBitmask[4]) << 32);
				heapCapacity |= (static_cast<uint64>(_rep._heap._totalByteCapacityBitmask[5]) << 40);
				heapCapacity |= (static_cast<uint64>(_rep._heap._totalByteCapacityBitmask[6]) << 48);
				heapCapacity |= (static_cast<uint64>(_flag) << 56);
...
#pragma intrinsic(_BitScanForward)
		...
		[[nodiscard]] static Option<StringIndex> avx512FindCharInString(const TestString& self, const char c) {
			...
			const uint64 iterationsToDo = (self._rep._heap._bytesOfBufferUsed) % 64 == 0 ? self._rep._heap._bytesOfBufferUsed : self._rep._heap._bytesOfBufferUsed + (64 - (self._rep._heap._bytesOfBufferUsed % 64));
		...
		[[nodiscard]] static Option<StringIndex> avx2FindCharInString(const TestString& self, const char c) {
			...
			const uint64 iterationsToDo = (self._rep._heap._bytesOfBufferUsed) % 32 == 0 ? self._rep._heap._bytesOfBufferUsed : self._rep._heap._bytesOfBufferUsed + (32 - (self._rep._heap._bytesOfBufferUsed % 32));
