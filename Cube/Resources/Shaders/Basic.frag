...
void main()
{
	...
	*/
	...
	/*
	// 0, 0
	const vec3 TopRightCol = vec3(
		float(vertCols.x & BYTE_1_BITMASK) / 255.f,
		float(vertCols.x >> 8 & BYTE_1_BITMASK) / 255.f,
		float(vertCols.x >> 16 & BYTE_1_BITMASK) / 255.f
	);
	// 1, 0
	const vec3 TopLeftCol = vec3(
		float(vertCols.x >> 24 & BYTE_1_BITMASK) / 255.f,
		float(vertCols.y & BYTE_1_BITMASK) / 255.f,
		float(vertCols.y >> 8 & BYTE_1_BITMASK) / 255.f
	);
	// 0, 1
	const vec3 BottomRightCol = vec3(
		float(vertCols.y >> 16 & BYTE_1_BITMASK) / 255.f,
		float(vertCols.y >> 24 & BYTE_1_BITMASK) / 255.f,
		float(vertCols.z & BYTE_1_BITMASK) / 255.f
	);
	// 1, 1
	const vec3 BottomLeftCol = vec3(
		float(vertCols.z >> 8 & BYTE_1_BITMASK) / 255.f,
		float(vertCols.z >> 16 & BYTE_1_BITMASK) / 255.f,
		float(vertCols.z >> 24 & BYTE_1_BITMASK) / 255.f
	);*/

	//const vec3 interpColor = InterpolateColor(TopRightCol, TopLeftCol, BottomRightCol, BottomLeftCol);


	//float r = float((testBuffer.scolor[1] & 255) / 255.f);
	//float g = float((testBuffer.scolor[1] >> 8 & 255) / 255.f);
	//float b = float((testBuffer.scolor[1] >> 16 & 255) / 255.f);
	//float a = float((testBuffer.scolor[1] >> 24 & 255) / 255.f);
