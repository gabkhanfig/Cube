...
static bool CanDrawFace(const MappedAdjacentAndBuriedChunks& adjacentChunks, WorldPosition position, BlockFacing face) {
  ...
  return !adjacentBlock->getTypeInfo()->isFullSolid;
...
void BlockVTable::Default::FullCubeMeshFunc(
	const Block* self, 
...
{
  const EBlockTexture allSideTexture = vTable->faceTexture.invoke(self, BlockFacing::Dir_Down);
